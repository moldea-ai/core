...
/** Installs and exercises the packed Anthropic adapter under the active Node.js runtime. */
const runRuntimeCompatibilityCheck = async (artifactDirectory) => {
  ...
  const consumerDirectory = await mkdtemp(
    path.join(tmpdir(), 'moldea-adapter-anthropic-runtime-consumer-'),
  );
  ...
  try {
    await writeFile(
      ...
      `${JSON.stringify({ name: 'adapter-anthropic-runtime-consumer', private: true, type: 'module' })}\n`,
      'utf8',
    );
