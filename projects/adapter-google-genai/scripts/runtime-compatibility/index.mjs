...
/** Installs and exercises the packed Google Gen AI adapter under the active Node.js runtime. */
const runRuntimeCompatibilityCheck = async (artifactDirectory) => {
  ...
  const consumerDirectory = await mkdtemp(
    path.join(tmpdir(), 'moldea-adapter-google-genai-runtime-consumer-'),
  );
  ...
  try {
    await writeFile(
      ...
      `${JSON.stringify({ name: 'adapter-google-genai-runtime-consumer', private: true, type: 'module' })}\n`,
      'utf8',
    );
