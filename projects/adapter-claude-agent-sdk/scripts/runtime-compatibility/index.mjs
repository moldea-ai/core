...
/** Installs and exercises the packed Claude Agent SDK adapter under the active Node.js runtime. */
const runRuntimeCompatibilityCheck = async (artifactDirectory) => {
  ...
  const consumerDirectory = await mkdtemp(
    path.join(tmpdir(), 'moldea-adapter-claude-agent-sdk-runtime-consumer-'),
  );
  ...
  try {
    await writeFile(
      ...
      `${JSON.stringify({ name: 'adapter-claude-agent-sdk-runtime-consumer', private: true, type: 'module' })}\n`,
      'utf8',
    );
