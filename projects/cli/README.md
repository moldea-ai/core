...
Version 3 bundles Anthropic adapter `2.0.0`, whose client-tool names must match `^[A-Za-z0-9_-]{1,64}$`.
...
The package-backed `anthropic` and `openai` runtime adapters are active for their experimental TypeScript Messages and Responses API targets. The `custom` adapter remains built into Core and requires no separate package.
...
The CLI does not expose selected paths, candidate paths, resolved repository roots, raw process errors, or Git diagnostics in failures. Every accepted snapshot operation creates a fresh Core instance with the exact effective limits: `maxEntries`, `maxFileBytes`, `maxManifestBytes`, `maxDiagnostics`, and `maxEvidence` retain their names, while `maxTotalBytes` becomes Core's `maxTotalBytesRead`. The active package-backed adapter set contains the immutable `anthropic` and `openai` adapter singletons; Core's built-in `custom` behavior remains available without registration.
