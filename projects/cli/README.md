...
The open-source skill repository exercises the real candidate tarballs through supported npm, pnpm, and Yarn versions and validates deterministic `compatibility --json` and `inspect --json` behavior without registry publication. Package-backed runtime adapters remain excluded until their own implementations, fixtures, and verified compatibility claims exist.
