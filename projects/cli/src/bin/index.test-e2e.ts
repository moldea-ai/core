...
describe('published CLI package and executable', () => {
  test('packs only the executable package surface and exact metadata', () => {
    ...
    expect(packResult).toMatchObject({ name: '@moldea.ai/cli', version: '1.1.0' });
    ...
    expectPackageManifest(manifest, 'workspace:1.0.1', 'workspace:1.0.0');
  ...
  test('rewrites exact workspace dependencies in the real tarball', () => {
    ...
    try {
      ...
      expectPackageManifest(manifest, '1.0.1', '1.0.0');
      expect(executable.startsWith('#!/usr/bin/env node\n')).toBe(true);
      expect(executable).toContain('@moldea.ai/adapter-openai');
      expect(executable).toContain('@moldea.ai/adapter-anthropic');
      expect(executable).toContain('minimumGitVersion');
  ...
  test('installs and executes the real CLI and foundational package tarballs', async () => {
    ...
    try {
      ...
      expect(jsonUsageFailure.stdout).toBe(
        '{"cliVersion":"1.1.0","command":null,"error":{"code":"INVALID_ARGUMENT","details":{},"message":"The command invocation is invalid.","path":null,"retryable":false,"source":"cli"},"result":null,"schemaVersion":1,"status":"error"}\n',
      );
...
`,
      ...
      expect(validJsonValidationCommand.stdout).toBe(
        '{"cliVersion":"1.1.0","command":"validate","error":null,"result":{"diagnostics":[],"formatVersion":1,"source":{"kind":"git-working-tree"}},"schemaVersion":1,"status":"valid"}\n',
      );
...
`,
      ...
      expect(missingGitResult.stdout).toBe(
        '{"cliVersion":"1.1.0","command":"validate","error":{"code":"GIT_NOT_FOUND","details":{},"message":"The Git executable is unavailable.","path":null,"retryable":false,"source":"git"},"result":null,"schemaVersion":1,"status":"error"}\n',
      );
