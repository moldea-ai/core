...
describe('published CLI package and executable', () => {
  test('packs only the executable package surface and exact metadata', () => {
    ...
    expect(packResult).toMatchObject({ name: '@moldea.ai/cli', version: '2.1.1' });
  ...
  test('rewrites exact workspace dependencies in the real tarball', () => {
    ...
    try {
      ...
      expectPackageManifest(manifest, '2.0.0', '1.0.1', '2.0.2', '1.0.1');
      expect(executable.startsWith('#!/usr/bin/env node\n')).toBe(true);
      expect(executable).toContain('@moldea.ai/adapter-openai');
      expect(executable).toContain('@moldea.ai/adapter-anthropic');
      expect(executable).toContain('minimumGitVersion');
  ...
  test('installs and executes the real CLI and foundational package tarballs', async () => {
    ...
    try {
      ...
      expect(humanCompatibility.stdout).toContain(
        'anthropic: active=yes, bundled=1.0.1, kind=package, package=@moldea.ai/adapter-anthropic, status=available\n',
      );
      expect(humanCompatibility.stdout).toContain(
        'openai: active=yes, bundled=2.0.2, kind=package, package=@moldea.ai/adapter-openai, status=available\n',
      );
      expect(humanCompatibility.stdout).toContain(
...
`,
      ...
      expect(validJsonValidationCommand.stdout).toBe(
        '{"cliVersion":"2.1.1","command":"validate","error":null,"result":{"diagnostics":[],"formatVersion":1,"source":{"kind":"git-working-tree"}},"schemaVersion":1,"status":"valid"}\n',
      );
...
`,
      ...
      expect(missingGitResult.stdout).toBe(
        '{"cliVersion":"2.1.1","command":"validate","error":{"code":"GIT_NOT_FOUND","details":{},"message":"The Git executable is unavailable.","path":null,"retryable":false,"source":"git"},"result":null,"schemaVersion":1,"status":"error"}\n',
      );
