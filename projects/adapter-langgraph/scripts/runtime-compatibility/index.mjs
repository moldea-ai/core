...
/** Installs and exercises the packed LangGraph adapter under the active Node.js runtime. */
const runRuntimeCompatibilityCheck = async (artifactDirectory) => {
  ...
  const consumerDirectory = await mkdtemp(
    path.join(tmpdir(), 'moldea-adapter-langgraph-runtime-consumer-'),
  );
  ...
  try {
    await writeFile(
      ...
      `${JSON.stringify({ name: 'adapter-langgraph-runtime-consumer', private: true, type: 'module' })}\n`,
      'utf8',
    );
