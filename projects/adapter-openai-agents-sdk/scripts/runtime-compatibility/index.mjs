...
/** Installs and exercises the packed OpenAI Agents SDK adapter under the active Node.js runtime. */
const runRuntimeCompatibilityCheck = async (artifactDirectory) => {
  ...
  const consumerDirectory = await mkdtemp(
    path.join(tmpdir(), 'moldea-adapter-openai-agents-sdk-runtime-consumer-'),
  );
  ...
  try {
    await writeFile(
      ...
      `${JSON.stringify({ name: 'adapter-openai-agents-sdk-runtime-consumer', private: true, type: 'module' })}\n`,
      'utf8',
    );
