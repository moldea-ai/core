...
/** Installs and exercises the packed Vercel AI SDK adapter under the active Node.js runtime. */
const runRuntimeCompatibilityCheck = async (artifactDirectory) => {
  ...
  const consumerDirectory = await mkdtemp(
    path.join(tmpdir(), 'moldea-adapter-vercel-ai-sdk-runtime-consumer-'),
  );
  ...
  try {
    await writeFile(
      ...
      `${JSON.stringify({ name: 'adapter-vercel-ai-sdk-runtime-consumer', private: true, type: 'module' })}\n`,
      'utf8',
    );
