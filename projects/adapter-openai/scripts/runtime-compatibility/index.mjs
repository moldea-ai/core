...
/** Installs and exercises the packed OpenAI adapter under the active Node.js runtime. */
const runRuntimeCompatibilityCheck = async (artifactDirectory) => {
  ...
  const consumerDirectory = await mkdtemp(
    path.join(tmpdir(), 'moldea-adapter-openai-runtime-consumer-'),
  );
  ...
  try {
    await writeFile(
      ...
      `${JSON.stringify({ name: 'adapter-openai-runtime-consumer', private: true, type: 'module' })}\n`,
      'utf8',
    );
