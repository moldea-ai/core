...
describe('runtime compatibility Markdown generation', () => {
  ...
  test('renders verified target maturity and every support section without inference', async () => {
    ...
    expect(markdown).toContain('Support level: `experimental`');
    ...
    expect(markdown).not.toContain('Support level: `supported`');
  });
});
