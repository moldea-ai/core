...
`@moldea.ai/repository`, `@moldea.ai/repository-fs`, `@moldea.ai/core`, `@moldea.ai/adapter-anthropic`, `@moldea.ai/adapter-claude-agent-sdk`, `@moldea.ai/adapter-google-genai`, `@moldea.ai/adapter-openai`, `@moldea.ai/adapter-openai-agents-sdk`, `@moldea.ai/adapter-vercel-ai-sdk`, `@moldea.ai/cli`, and `@moldea.ai/website-ui` form the available package set. Repository and Core provide the source-neutral reader and universal interpretation foundations, Repository FS supplies the coherent local-filesystem reader, the provider and agent-SDK adapters contribute static evidence for their experimental targets, the CLI composes them into the complete version `1` read-only executable, and Website UI provides the shared Astro and Tailwind foundations for moldea public websites. The built-in `custom` runtime and package-backed `anthropic`, `claude-agent-sdk`, `google-genai`, `openai`, `openai-agents-sdk`, and `vercel-ai-sdk` runtimes are verified as available; the remaining package-backed adapters stay `planned`. Real tarball installation and execution remain the release boundary for every package version.
