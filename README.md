...
`@moldea.ai/repository` and `@moldea.ai/core` now provide the source-neutral reader and universal interpretation foundations. `@moldea.ai/repository-fs` currently exposes its option and resource-limit foundation while strict exact-path and recursive-directory inventory behavior, private filesystem fingerprints, creation-time verification, and frozen lookup and listing are built behind that boundary. Verified file capture, immutable caching, invalidation, operation coordination, and the cohesive public factory remain required before the filesystem reader is exposed. The canonical Runtime Compatibility Matrix is now materialized with every approved adapter marked `planned`; this publishes the inventory without claiming runtime support before adapter implementations and fixtures exist.
